...
@section('dashboard-content')
    <a href="{{ route('roles.index') }}" class="btn btn-sky max-w-fit mb-5">
    ...
    <h2 class="mb-5 text-left">{{ __('Create role') }}</h2>

    @include('components.error-message')

    <form action="{{ route('roles.store') }}" method="POST" class="flex flex-col">
        @csrf
