...
@section('dashboard-content')
    <a href="{{ route('representatives.index') }}" class="btn btn-sky max-w-fit mb-5">
    ...
    <h2 class="mb-5 text-left">{{ __('Edit representative') }}</h2>

    @include('components.error-message')

    <div class="field-card">
        <form action="{{ route('representatives.update', $representative->id) }}" method="POST" class="flex flex-col">
            @method('PUT')
            @csrf
        
            @include('representative.form', ['representative' => $representative])
