...
@section('dashboard-content')
    ...
    <h2 class="mb-5 text-left">{{ __('Edit status') }}</h2>

    @include('components.error-message')

    <form action="{{ route('project-statuses.update', $projectStatus->id) }}" method="POST" class="flex flex-col">
        @method('PUT')
        @csrf
    
        @include('project-status.form', ['projectStatus' => $projectStatus])
